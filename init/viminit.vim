...
noremap <leader>R :AsyncRun! -cwd=<root> rg -n --no-heading 
			\ --color never -g *.h -g *.c* -g *.py -g *.js -g *.vim 
			\ <C-R><C-W> "<root>"


command! -nargs=1 MyGrep execute 'AsyncRun -cwd=<root> rg -n --no-heading --color never -g *.h -g *.c* -g *.py -g *.js -g *.vim '.<q-args>.' <root>'

function! MyGrepAndOpen(str)
	exec "MyGrep".a:str
	:call asyncrun#quickfix_toggle(11)
endfunc

command! -nargs=1 MyGrep2 :call MyGrepAndOpen('<q-args>')
	
noremap <leader>r :MyGrep2 
...
"----------------------------------------------------------------------
if has('gui_running')
	noremap <silent><m-e> :tabnew<cr>
	inoremap <silent><m-e> <ESC>:tabnew<cr>
